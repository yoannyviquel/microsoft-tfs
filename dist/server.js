...
var getReleaseDefinition = {
  name: "tfs_getreleasedefinition",
  description: "Fetches the detail of a single Microsoft TFS release definition (deployment pipeline) by ID. Exposes its artifact aliases and environment names \u2014 useful to prepare a tfs_createrelease (which alias / which environments to set manual).",
  inputSchema: {
    type: "object",
    properties: {
      project: {
  ...
  handler: async (args, { client }) => {
    ...
    try {
      ...
      const url = client.url(
        projectPath4(project, `/_apis/release/definitions/${definitionId}`),
        { "api-version": "5.1-preview.3" }
      );
      ...
      const lines = [];
      lines.push(
        `\u{1F680} **Release definition ${definition.name ?? definitionId} (ID ${definition.id ?? definitionId})**`
      );
      lines.push("");
      if (definition.path && definition.path !== "\\") {
        lines.push(`\u{1F4C2} **Path:** ${definition.path}`);
      ...
      lines.push("");
      lines.push(
        `\u{1F4A1} Create a release from a build with \`tfs_createrelease\` (definitionId ${definition.id ?? definitionId}).`
      );
      return lines.join("\n") + "\n";
    } catch (err) {
...
var createRelease = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      artifactAlias: {
        type: "string",
        description: "Alias of the artifact to set the build on (optional). If omitted, it is resolved from the definition: the artifact with isPrimary === true, otherwise the first one."
      },
      description: {
        type: "string",
      ...
      manualEnvironments: {
        type: "array",
        items: { type: "string" },
        description: 'Names of environments to NOT deploy automatically (set to manual), e.g. ["Prod_BDX","Prod_PAR"] (optional)'
      }
    },
  ...
  handler: async (args, { client }) => {
    ...
    try {
      ...
      if (!artifactAlias) {
        ...
        const chosen = artifacts.find((a) => a.isPrimary === true) ?? artifacts[0];
...
`;
      ...
      result += `
\u{1F4A1} Deploy a manual stage with \`tfs_deployrelease\`, or track it with \`tfs_getreleases\`.`;
      return result;
    } catch (err) {
